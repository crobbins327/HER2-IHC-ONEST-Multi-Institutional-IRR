...
opa = function(df, perAgree = 100, removeNA=TRUE){
  ...
  if (pathAgree==m){ 
    ...
    for(j in 1:n){
      comp = as.vector(unlist(df[j,]))
      #Find counts of all categories of ratings for sample, and select the count with the most ratings (highest agreement)
      #Takes a long time to calculate on one processor core... how to speed up and parallelize?
      if(removeNA){
...
ONEST_plotModel_fromData = function(model_data, name, file, percent=TRUE, xBy=2){
  ...
  q = ggplot(data = consist) + 
    ...
    xlab('Number of Pathologists in Group') + ylab(cons_ylab) + ggtitle(name) +
    labs(color='') +
  ...
  p = ggplot(data = diff) + 
    ...
    xlab('Number of Pathologists in Group') + ylab('OPA Difference between points') + ggtitle(name) +
    labs(color='') +
    scale_linetype_manual(values=line_types) +
    scale_color_manual(values=color_codes, guide = guide_legend(reverse = TRUE)) +
    guides(linetype="none")+
...
ONEST_plot_fromData = function(concord, plot_data, name, file, ylab = 'Overall Percent Agreement', color='black', percent=TRUE){
  ...
  p = ggplot(data = df, aes(x=path_number, y=value, group=iter)) + geom_line(color=color, alpha=0.4, size=0.5) + geom_point(color=color, size=3, pch=21) + 
    ...
    xlab('Number of Pathologists in Group') + ylab(ylab) + ggtitle(name) +
    theme_linedraw() +
    theme(text=element_text(size=14),
          axis.text = element_text(size=14, color='black'),
          axis.title = element_text(size=16, color='black'),
